...
export class ForfaitsComponent {
    ...
    this.aR.params.subscribe(async (params) => {
      ...
      await this.payS
        ...
        .subscribe(async (data: any) => {
          ...
          await this.clientS
          .getClientType()
          .subscribe(async (ct: any) => {
            const clientType = ct.filter((c) => c._id === this.client.idClientType);
  ...
  encaisser() {
    ...
    this.payS.postClientForfait(this.choosenForfait, this.idUser).subscribe((data: any) => {
  ...
  deleteForfait(id) {
    this.choosenForfait = this.choosenForfait.filter((element) => element.idForfait !== id)
    this.myForfaits = this.myForfaits.filter((element) => element._id !== id) 
  }
  delete(id) {
    this.isdeleting= true;
    this.deleteForfait=id;
