...
export class ActivityComponent implements OnInit {
  ...
  displayedColumns = ['client', 'boiteNumber', 'boiteType', 'total', 'staff'];
  errorMessage = false;
  datasource;
  date; // date filtré
  allPayments: Array<{
    ...
    idStaff: string, idClient: string
  }> = []; // tous les clients
  ...
  async getData() {
    await this.payS.getAllPayment().subscribe(async (payments: any) => {
      ...
      await this.boiteS.getBoites().subscribe(async (boites: any) => {
        this.allPayments.forEach(async pay => {
          await boites.forEach(boite => {
            if (pay.boiteNumber === boite.number) {
              pay.boiteType = boite.boiteType;
            }
          });
        });
        await this.clientS.getClients().subscribe(async (clients: any) => {
          await this.allPayments.forEach(async pay => {
            await clients.forEach(client => {
              if (pay.idClient === client._id) {
                pay.clientName = client.name;
              }
            });
          });
        });
